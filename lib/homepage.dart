...
class _Homepage extends State<HomePage> {
  ...
  // ignore: non_constant_identifier_names
  var confirmed_cases = SortedMap(Ordering.byValue()),
      active_cases = SortedMap(Ordering.byValue()),
      recovered_cases = SortedMap(Ordering.byValue()),
      deaths_cases = SortedMap(Ordering.byValue());
  ...
  void initState() {
    if (widget.india_cases.isEmpty) {
      _get_state_data().then((jsonResponse) {
        setState(() {
          this.india_cases.addAll([
            jsonResponse["TT"]["total"]["confirmed"],
            jsonResponse["TT"]["total"]["deceased"],
            jsonResponse["TT"]["total"]["recovered"],
            jsonResponse["TT"]["total"]["confirmed"] -
                jsonResponse["TT"]["total"]["deceased"] -
                jsonResponse["TT"]["total"]["recovered"]
          ]);
        ...
        widget.state_codes.forEach((element) {
          ...
          if (state != null) {
            ...
            var active = state["total"]["confirmed"] -
                state["total"]["deceased"] -
                state["total"]["recovered"];
            var recovered = state["total"]["recovered"];
            var deaths = state["total"]["deceased"];
            setState(() {
              this.confirmed_cases.addAll({
                widget.state_names[widget.state_codes.indexOf(element)]:
                    confirmed
              });
              this.active_cases.addAll({
                widget.state_names[widget.state_codes.indexOf(element)]: active
              });
              this.recovered_cases.addAll({
                widget.state_names[widget.state_codes.indexOf(element)]:
                    recovered
              });
              this.deaths_cases.addAll({
                widget.state_names[widget.state_codes.indexOf(element)]: deaths
              });
            });
          }
        });
                      ...
                      child: Row(children: [
                        ...
                        Container(
                          child: Center(
                            child: Text(
                                "Confirmed \n " +
                                    f.format(this.india_cases[0]).toString(),
                                style: GoogleFonts.oswald(
                  ...
                  FlexWidget(
                    child: Container(
                      child: Row(children: [
                        Container(
                          child: Center(
                            child: Icon(FlutterIcons.redo_faw5s,
                                color: Colors.green),
                          ),
                          padding: EdgeInsets.all(5),
                          margin: EdgeInsets.fromLTRB(10, 5, 10, 5),
                        ...
                        Container(
                          child: Center(
                            child: Text(
                                "Recovered \n " +
                                    f.format(india_cases[2]).toString(),
                                style: GoogleFonts.oswald(
                  ...
                  FlexWidget(
                    child: Container(
                      child: Row(children: [
                        Container(
                          child: Center(
                            child: Icon(FlutterIcons.syringe_faw5s,
                                color: Colors.blue),
                          ),
                          padding: EdgeInsets.all(5),
                          margin: EdgeInsets.fromLTRB(10, 5, 10, 5),
                        ...
                        Container(
                          child: Center(
                            child: Text(
                                "  Active \n " +
                                    f.format(this.india_cases[3]).toString(),
                                style: GoogleFonts.oswald(
                  ...
                  FlexWidget(
                    child: Container(
                      child: Row(children: [
                        Container(
                          child: Center(
                            child: Icon(FlutterIcons.heart_broken_faw5s,
                                color: Colors.grey),
                          ),
                          padding: EdgeInsets.all(5),
                          margin: EdgeInsets.fromLTRB(10, 5, 10, 5),
                        ...
                        Container(
                          child: Center(
                            child: Text(
                                "  Deaths \n " +
                                    f.format(india_cases[1]).toString(),
                                style: GoogleFonts.oswald(
                ...
                child: SfCircularChart(
                    legend: Legend(
                      ...
                      legendItemBuilder: (String name, dynamic series,
                        ...
                        return Container(
                          child: Text(
                            point.x + '-' + point.y.toStringAsFixed(2) + '%',
                            style: GoogleFonts.oswald(
                                color: point.color, fontSize: 13),
                            textAlign: TextAlign.center,
                          ),
                          width: 120,
